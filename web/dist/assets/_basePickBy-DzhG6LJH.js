import {
  c as b,
  e as m,
  k as O,
  g as P,
  h as p,
  j as w,
  l as o,
  t as c,
  m as A,
} from './_baseUniq-CHUFGwGn.js';
import {
  aP as x,
  aQ as g,
  aC as N,
  aR as E,
  aS as F,
  aT as M,
  aU as I,
  aV as T,
  aW as _,
  aX as $,
  aY as y,
  aZ as B,
} from './index-CoIn5CRw.js';
var S = /\s/;
function R(n) {
  for (var r = n.length; r-- && S.test(n.charAt(r)); );
  return r;
}
var C = /^\s+/;
function G(n) {
  return n && n.slice(0, R(n) + 1).replace(C, '');
}
var v = NaN,
  H = /^[-+]0x[0-9a-f]+$/i,
  L = /^0b[01]+$/i,
  W = /^0o[0-7]+$/i,
  X = parseInt;
function Y(n) {
  if (typeof n == 'number') return n;
  if (x(n)) return v;
  if (g(n)) {
    var r = typeof n.valueOf == 'function' ? n.valueOf() : n;
    n = g(r) ? r + '' : r;
  }
  if (typeof n != 'string') return n === 0 ? n : +n;
  n = G(n);
  var t = L.test(n);
  return t || W.test(n) ? X(n.slice(2), t ? 2 : 8) : H.test(n) ? v : +n;
}
var q = 1 / 0,
  z = 17976931348623157e292;
function K(n) {
  if (!n) return n === 0 ? n : 0;
  if (((n = Y(n)), n === q || n === -1 / 0)) {
    var r = n < 0 ? -1 : 1;
    return r * z;
  }
  return n === n ? n : 0;
}
function Q(n) {
  var r = K(n),
    t = r % 1;
  return r === r ? (t ? r - t : r) : 0;
}
function fn(n) {
  var r = n == null ? 0 : n.length;
  return r ? b(n) : [];
}
var l = Object.prototype,
  U = l.hasOwnProperty,
  dn = N(function (n, r) {
    n = Object(n);
    var t = -1,
      e = r.length,
      a = e > 2 ? r[2] : void 0;
    for (a && E(r[0], r[1], a) && (e = 1); ++t < e; )
      for (var f = r[t], i = F(f), s = -1, d = i.length; ++s < d; ) {
        var u = i[s],
          h = n[u];
        (h === void 0 || (M(h, l[u]) && !U.call(n, u))) && (n[u] = f[u]);
      }
    return n;
  });
function un(n) {
  var r = n == null ? 0 : n.length;
  return r ? n[r - 1] : void 0;
}
function Z(n) {
  return function (r, t, e) {
    var a = Object(r);
    if (!I(r)) {
      var f = m(t);
      (r = O(r)),
        (t = function (s) {
          return f(a[s], s, a);
        });
    }
    var i = n(r, t, e);
    return i > -1 ? a[f ? r[i] : i] : void 0;
  };
}
var D = Math.max;
function J(n, r, t) {
  var e = n == null ? 0 : n.length;
  if (!e) return -1;
  var a = t == null ? 0 : Q(t);
  return a < 0 && (a = D(e + a, 0)), P(n, m(r), a);
}
var hn = Z(J);
function V(n, r) {
  var t = -1,
    e = I(n) ? Array(n.length) : [];
  return (
    p(n, function (a, f, i) {
      e[++t] = r(a, f, i);
    }),
    e
  );
}
function gn(n, r) {
  var t = T(n) ? _ : V;
  return t(n, m(r));
}
var j = Object.prototype,
  k = j.hasOwnProperty;
function nn(n, r) {
  return n != null && k.call(n, r);
}
function mn(n, r) {
  return n != null && w(n, r, nn);
}
function rn(n, r) {
  return n < r;
}
function tn(n, r, t) {
  for (var e = -1, a = n.length; ++e < a; ) {
    var f = n[e],
      i = r(f);
    if (i != null && (s === void 0 ? i === i && !x(i) : t(i, s)))
      var s = i,
        d = f;
  }
  return d;
}
function vn(n) {
  return n && n.length ? tn(n, $, rn) : void 0;
}
function an(n, r, t, e) {
  if (!g(n)) return n;
  r = o(r, n);
  for (var a = -1, f = r.length, i = f - 1, s = n; s != null && ++a < f; ) {
    var d = c(r[a]),
      u = t;
    if (d === '__proto__' || d === 'constructor' || d === 'prototype') return n;
    if (a != i) {
      var h = s[d];
      (u = void 0), u === void 0 && (u = g(h) ? h : y(r[a + 1]) ? [] : {});
    }
    B(s, d, u), (s = s[d]);
  }
  return n;
}
function on(n, r, t) {
  for (var e = -1, a = r.length, f = {}; ++e < a; ) {
    var i = r[e],
      s = A(n, i);
    t(s, i) && an(f, o(i, n), s);
  }
  return f;
}
export {
  rn as a,
  tn as b,
  V as c,
  on as d,
  vn as e,
  fn as f,
  hn as g,
  mn as h,
  dn as i,
  Q as j,
  un as l,
  gn as m,
  K as t,
};
