import { s as W, S as N, a as P } from './chunk-AEK57VVT-SnLTeNxd.js';
import {
  _ as u,
  e as t,
  k as T,
...
import { G as J } from './graph-ByJpsr8c.js';
import { l as X } from './layout-DShoTQlJ.js';
import './chunk-RZ5BOZE2-Dl5yENMm.js';
...
var L = {},
  ...
  Y = u((e) => L[e], 'get'),
  G = u(() => Object.keys(L), 'keys'),
  I = u(() => G().length, 'size'),
  $ = { get: Y, set: D, keys: G, size: I },
  j = u(
    (e) =>
      e
        .append('circle')
  ...
  Z = u((e, i) => {
    ...
    return (
      e
        ...
        .attr('width', o.width + 2 * t().state.padding)
        .attr('height', o.height + 2 * t().state.padding)
        .attr('rx', t().state.radius),
      d
    );
  }, 'drawSimpleState'),
  K = u((e, i) => {
    const d = u(function (l, B, m) {
        const v = l
          .append('tspan')
          .attr('x', 2 * t().state.padding)
          .text(B);
    ...
    return (
      ...
      e
        ...
        .attr('height', p.height + g + 2 * t().state.padding)
        .attr('rx', t().state.radius),
      e
    );
  }, 'drawDescrState'),
  Q = u((e, i, d) => {
    const o = t().state.padding,
      r = 2 * t().state.padding,
      g = e.node().getBBox(),
      x = g.width,
      a = g.x,
    ...
    i.doc,
      (l = a - o),
      p > x && (l = (x - c) / 2 + o),
      Math.abs(a - B.x) < o && p > x && (l = a - (p - x) / 2);
  ...
  et = u((e, i, d, o) => {
    ...
    for (const y of a) {
      ...
      if (p.length > 0) {
        ...
        (r += s),
          c.attr('x', i + t().state.noteMargin),
          c.attr('y', d + r + 1.25 * t().state.noteMargin);
      }
    }
  ...
  at = u((e, i) => {
    ...
    const d = i.append('rect').attr('x', 0).attr('y', t().state.padding),
      o = i.append('g'),
      { textWidth: r, textHeight: g } = et(e, 0, 0, o);
    return (
      d.attr('height', g + 2 * t().state.noteMargin),
      d.attr('width', r + t().state.noteMargin * 2),
      d
    );
  }, 'drawNote'),
  _ = u(function (e, i) {
    const d = i.id,
      o = { id: d, label: i.id, width: 0, height: 0 },
      r = e.append('g').attr('id', d).attr('class', 'stateGroup');
    i.type === 'start' && j(r),
      i.type === 'end' && V(r),
      (i.type === 'fork' || i.type === 'join') && tt(r, i),
      i.type === 'note' && at(i.note.text, r),
      i.type === 'divider' && q(r),
      i.type === 'default' && i.descriptions.length === 0 && Z(r, i),
      i.type === 'default' && i.descriptions.length > 0 && K(r, i);
    const g = r.node().getBBox();
    return (
      (o.width = g.width + 2 * t().state.padding),
      (o.height = g.height + 2 * t().state.padding),
      $.set(d, o),
      o
    );
  }, 'drawState'),
  ...
  it = u(function (e, i, d) {
    ...
    if (
      ...
      const s = e.append('g').attr('class', 'stateLabel'),
        { x: y, y: p } = O.calcLabelPosition(i.points),
        c = z.getRows(d.title);
      let l = 0;
      const B = [];
      let m = 0,
      ...
      for (let f = 0; f <= c.length; f++) {
        const h = s
            ...
            .attr('y', p + l),
          w = h.node().getBBox();
        (m = Math.max(m, w.width)),
          (v = Math.min(v, w.x)),
          S.info(w.x, y, p + l),
          l === 0 &&
            ((l = h.node().getBBox().height), S.info('Title height', l, p)),
          B.push(h);
      }
      let E = l * c.length;
      if (c.length > 1) {
      ...
      s
        .insert('rect', ':first-child')
        .attr('class', 'box')
        .attr('x', y - m / 2 - t().state.padding / 2)
        .attr('y', p - E / 2 - t().state.padding / 2 - 3.5)
  ...
  st = u(function (e, i, d, o) {
    b = t().state;
    const r = t().securityLevel;
    let g;
    r === 'sandbox' && (g = T('#i' + i));
    const x =
        r === 'sandbox' ? T(g.nodes()[0].contentDocument.body) : T('body'),
      a = r === 'sandbox' ? g.nodes()[0].contentDocument : document;
    ...
    const p = b.padding,
      ...
      B = c.height + p * 2,
      m = l * 1.75;
    C(s, B, m, b.useMaxWidth),
      s.attr(
        'viewBox',
        `${c.x - b.padding}  ${c.y - b.padding} ` + l + ' ' + B,
      );
  }, 'draw'),
  dt = u((e) => (e ? e.length * b.fontSizeFactor : 1), 'getLabelWidth'),
  R = u((e, i, d, o, r, g, x) => {
    const a = new J({ compound: !0, multigraph: !0 });
    let s,
      y = !0;
    ...
    for (const n of l) {
      ...
      if (f.doc) {
        ...
        h = R(f.doc, w, f.id, !o, r, g, x);
        {
          ...
          (h.width = k.width),
            (h.height = k.height + b.padding / 2),
            (H[f.id] = { y: b.compositTitleSize });
        }
      } else h = _(i, f, a);
      if (f.note) {
        const w = {
          ...
          k = _(i, w, a);
        f.note.position === 'left of'
          ? (a.setNode(h.id + '-note', k), a.setNode(h.id, h))
          : (a.setNode(h.id, h), a.setNode(h.id + '-note', k)),
    ...
    a.nodes().forEach(function (n) {
      n !== void 0 && a.node(n) !== void 0
        ? (S.warn('Node ' + n + ': ' + JSON.stringify(a.node(n))),
          r
            .select('#' + m.id + ' #' + n)
            .attr(
              'transform',
              'translate(' +
                (a.node(n).x - a.node(n).width / 2) +
                ',' +
                (a.node(n).y + (H[n] ? H[n].y : 0) - a.node(n).height / 2) +
                ' )',
            ),
          r
            .select('#' + m.id + ' #' + n)
            .attr('data-x-shift', a.node(n).x - a.node(n).width / 2),
          g
            .querySelectorAll('#' + m.id + ' #' + n + ' .divider')
            .forEach((h) => {
              const w = h.parentElement;
              let k = 0,
                M = 0;
              w &&
                (w.parentElement && (k = w.parentElement.getBBox().width),
                (M = parseInt(w.getAttribute('data-x-shift'), 10)),
                Number.isNaN(M) && (M = 0)),
                h.setAttribute('x1', 0 - M + 8),
                h.setAttribute('x2', k - M - 8);
            }))
    ...
    a.edges().forEach(function (n) {
      n !== void 0 &&
        a.edge(n) !== void 0 &&
        (S.debug(
          'Edge ' + n.v + ' -> ' + n.w + ': ' + JSON.stringify(a.edge(n)),
        ),
        it(i, a.edge(n), a.edge(n).relation));
    }),
      (v = m.getBBox());
